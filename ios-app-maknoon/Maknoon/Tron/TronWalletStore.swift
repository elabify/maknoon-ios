...
enum TronWalletKind: Codable, Hashable, Sendable {
    ...
    private enum HardwareKeys: String, CodingKey { case deviceId, account, addressBase58Check }
    private enum FlatKeys: String, CodingKey { case type, account, deviceId, addressBase58Check }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .hardware(let deviceId, let account, let addr):
            ...
            try h.encode(addr, forKey: .addressBase58Check)
        }
    }
    ...
    init(from decoder: Decoder) throws {
        if let c = try? decoder.container(keyedBy: CaseKey.self), !c.allKeys.isEmpty {
            ...
            if let h = try? c.nestedContainer(keyedBy: HardwareKeys.self, forKey: .hardware) {
                self = .hardware(
                    ...
                    addressBase58Check: try h.decode(String.self, forKey: .addressBase58Check)
                ); return
            }
        }
        ...
        if (try? f.decode(String.self, forKey: .type)) == "hardware" {
            self = .hardware(
                ...
                addressBase58Check: try f.decode(String.self, forKey: .addressBase58Check)
            )
        } else {
...
final class TronWalletStore {
    ...
    /// networks. The Add sheet seeds the Account stepper with this so
    ...
    func relink(walletId: UUID, toDeviceId deviceId: UUID) {
        ...
        guard case let .hardware(_, account, addressBase58Check) = wallets[idx].kind else { return }
        wallets[idx].kind = .hardware(deviceId: deviceId, account: account, addressBase58Check: addressBase58Check)
        persist()
    }
